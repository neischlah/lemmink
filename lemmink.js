...
const common = {
  ...
  addElementAfter(element, newElement) {
...
const relativeChannel = {
  decorate() {
    ...
    if (!lemmyHostSet.has(hostname)) {
      console.debug(
        "Lemmink: The current hostname is not a known Lemmy instance"
      );
      return;
    }
    for (const link of common.getElements("//a[starts-with(@href, '/c/')]")) {
      let matches = link.pathname.match(/^\/c\/([^/]*)@?([^/]*)/);
      if (!matches || !matches[1]) continue;
      let href = `https://${mainInstance}/c/${matches[1]}@${
        matches[2] || hostname
      }`;
      link.setAttribute("lemminked", "");
      common.addElementAfter(link, common.getLinkElement(href));
    }
  },
};

const exclamationLink = {
  decorate() {
    for (const link of common.getElements("//a[starts-with(text(), '!')]")) {
      let target = this.getTarget(link.textContent);
      if (!target || target.instance === mainInstance) continue;
      let href = `https://${mainInstance}/c/${target.channel}@${target.instance}`;
      common.addElementAfter(link, common.getLinkElement(href));
    }
  },
...
let lemmyUrlPromise = browser.storage.sync.get("lemmyUrl").then((res) => {
  let url = res?.lemmyUrl || "feddit.eu";
  mainInstance = url.replace(/(?:^https?:\/\/|[\/]+$)/g, "");
});

addEventListener("load", async () => {
  await lemmyUrlPromise;
  relativeChannel.decorate();
  exclamationLink.decorate();
});
